...
def test_retriever_role_filters_joinee():
    ...
    with patch("rag.retriever.get_chroma_collection", return_value=mock_collection), \
         patch("rag.retriever.get_embeddings", return_value=mock_embeddings), \
         patch("rag.retriever.get_ranker", return_value=False): # Bypass reranker
         
        # When user is joinee
        results, degraded = retrieve_and_rerank("hello query", "joinee", query_vector=[0.1, 0.2])
...
def test_retriever_role_filters_hr():
    ...
    with patch("rag.retriever.get_chroma_collection", return_value=mock_collection), \
         patch("rag.retriever.get_embeddings", return_value=mock_embeddings), \
         patch("rag.retriever.get_ranker", return_value=False):
         
        # When user is HR
        results, degraded = retrieve_and_rerank("hello query", "HR", query_vector=[0.1, 0.2])
...
def test_retriever_role_filters_admin_bypass():
    ...
    with patch("rag.retriever.get_chroma_collection", return_value=mock_collection), \
        ...
        # When user is admin
        results, degraded = retrieve_and_rerank("hello query", "admin", query_vector=[0.1, 0.2])
...
def test_retriever_embedding_generation_fallback():
    ...
    with patch("rag.retriever.get_chroma_collection", return_value=mock_collection), \
        ...
        results, degraded = retrieve_and_rerank("hello query", "joinee", query_vector=None)
