...
def parse_token_usage(response) -> Tuple[int, int, float]:
    ...
    try:
        ...
        if usage:
            ...
            input_tok = (
                usage_legacy.get("prompt_tokens", 0)
                or usage_legacy.get("input_tokens", 0)
                or 0
            )
            output_tok = (
                usage_legacy.get("completion_tokens", 0)
                or usage_legacy.get("output_tokens", 0)
                or 0
            )

        cost = (input_tok * GEMINI_INPUT_COST_M / 1_000_000) + (
            output_tok * GEMINI_OUTPUT_COST_M / 1_000_000
        )
...
def update_cost_metrics(
    state: AgentState, input_tokens: int, output_tokens: int, cost: float
) -> Dict[str, Any]:
...
def cache_node(state: AgentState) -> Dict[str, Any]:
    ...
    if hit:
        logger.info(
            f"Cache hit verified for role {user_role}. Bypassing LangGraph processing."
        )
        return {
    ...
    logger.info(
        f"Cache miss for role {user_role}. Routing query to safety/LLM pipelines."
    )
    return {
...
def intent_node(state: AgentState) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"intent": intent, "token_usage": usage}
    except Exception as e:
        logger.error(
            f"Error during intent classification: {e}. Defaulting to 'General'."
        )
...
def rag_node(state: AgentState) -> Dict[str, Any]:
    ...
    chunks, degraded = retrieve_and_rerank(
        message, user_role, query_vector=query_vector, num_chunks=depth
    )
    ...
    for idx, c in enumerate(chunks, 1):
        ...
        source_docs.append(
            {"index": idx, "source": f"{source_name}{page_indicator}", "text": text}
        )
        context_str += (
            f"[{idx}] Source: {source_name}{page_indicator}\nContent: {text}\n\n"
        )
    ...
    try:
        ...
        ans = data.get(
            "answer", "I don't know the answer based on the onboarding files."
        )
...
def escalate_node(state: AgentState) -> Dict[str, Any]:
    ...
    try:
        ...
        summary = data.get(
            "escalation_summary", "Unable to answer onboarding question."
        )
...
def respond_node(state: AgentState) -> Dict[str, Any]:
    ...
    if len(history) > 10:
        logger.info(
            f"Conversation history size is {len(history)}. Compressing history using summarization."
        )
        ...
        try:
            ...
            if summary_text:
                ...
                logger.warning(
                    "Empty summary generated. Falling back to naive truncation."
                )
                history = history[-10:]
        except Exception as e:
            logger.error(
                f"Error summarizing history: {e}. Falling back to naive truncation."
            )
            history = history[-10:]
    ...
    if route_decision != "cache_hit" and rag_ans and confidence >= 0.8:
        ...
        if query_vector:
            logger.info(
                f"Caching high confidence answer (Score: {confidence}, Role: {user_role}) in L1 & L2."
            )
            add_to_caches(query, final_resp, source_docs, query_vector, user_role)
        else:
            logger.warning(
                "Could not cache high confidence answer due to missing embedding vector."
            )

    return {"final_response": final_resp, "conversation_history": history}
