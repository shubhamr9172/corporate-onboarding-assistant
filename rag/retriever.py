...
def get_chroma_collection():
    ...
    if _faq_collection is None:
        try:
            ...
            logger.error(
                f"Failed to connect to ChromaDB collection: {e}. Fallback triggered."
            )
            _faq_collection = None
...
def get_ranker():
    ...
    if _ranker is None:
        try:
            ...
            logger.warning(
                f"Failed to initialize FlashRank: {e}. Rerank step will be bypassed."
            )
            _ranker = False
    return _ranker
                ...
                matched_chunks.append(
                    {
                        "text": sect_clean,
                        "meta": {
                            "source": "onboarding_faq.txt (Offline Fallback)",
                            "score_weight": score,
                        },
                    }
                )
...
def retrieve_and_rerank(
    ...
    try:
        ...
        for i in range(len(results["documents"][0])):
            ...
            passages.append({"id": f"doc_{i}", "text": doc_text, "meta": meta})
        ...
        if ranker is False or ranker is None:
            ...
            output = [
                {"text": p["text"], "meta": p["meta"]} for p in passages[:num_chunks]
            ]
            return output, False
        ...
        for res in rerank_results[:num_chunks]:
            output_chunks.append({"text": res["text"], "meta": res["meta"]})

        return output_chunks, False

    except Exception as e:
        logger.error(
            f"Error during vector store retrieval pipeline: {e}. Triggering fallback."
        )
        return fallback_local_retrieval(query), True
