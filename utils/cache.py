    ...
    if _chroma_cache_collection is None:
        try:
            ...
            _chroma_cache_collection = client.get_or_create_collection(
                name="onboarding_semantic_cache", metadata={"hnsw:space": "cosine"}
            )
        except Exception as e:
...
def set_l1_cache(
    query: str, response: str, source_docs: List[Dict[str, Any]], user_role: str
):
...
def get_l2_cache(
    query: str, query_vector: List[float], user_role: str
    ...
    try:
        ...
        if results and results["documents"] and len(results["documents"][0]) > 0:
            ...
            if similarity >= SIMILARITY_THRESHOLD:
                logger.info(
                    f"L2 Cache Hit! (Role: {user_role}, Similarity: {similarity:.4f})"
                )
