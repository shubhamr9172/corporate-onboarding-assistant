...
def check_configuration() -> bool:
    ...
    if langsmith_tracing == "true" and not langsmith_key:
        logger.warning(
            "LANGCHAIN_TRACING_V2 is set to 'true', but LANGSMITH_API_KEY is missing. Tracing will fail."
        )
    ...
    if not redis_url:
        logger.warning(
            "REDIS_URL is not set. Cache & Rate Limiting will be bypassed (failed open)."
        )
    else:
        try:
            import redis
            ...
            logger.warning(
                "The 'redis' library is not installed. Cache & Rate Limiting will be bypassed."
            )
        except Exception as e:
            logger.warning(
                f"Connection to Redis failed: {e}. Cache & Rate Limiting will be bypassed (failed open)."
            )
