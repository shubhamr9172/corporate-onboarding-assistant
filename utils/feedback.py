...
def log_user_feedback(
    ...
    if run_id and os.getenv("LANGSMITH_API_KEY") and os.getenv("LANGCHAIN_TRACING_V2") == "true":
        try:
            ...
            logger.info(f"LangSmith run {run_id} annotated with user feedback score: {score}.")
        except Exception as e:
...
def update_benchmark_from_feedback():
    ...
    try:
        conn = sqlite3.connect(FEEDBACK_DB_PATH)
        cursor = conn.cursor()
        cursor.execute("SELECT DISTINCT query FROM feedback_logs WHERE rating = -1 ORDER BY id DESC LIMIT 50")
    ...
    for q in negative_queries:
        if len(dataset) >= MAX_DATASET_SIZE:
            logger.warning(f"Benchmark dataset reached maximum limit of {MAX_DATASET_SIZE} cases. Skipping additions.")
            break
    ...
    if new_cases_added > 0:
        try:
            ...
            logger.info(f"Added {new_cases_added} feedback regression cases to DeepEval test suite (Total cases: {len(dataset)}).")
        except Exception as e:
