...
TOXIC_KEYWORDS = [
    "bastard", "idiot", "dumb", "fool", "shit", "fuck", "bitch"
]
...
def validate_input(message: str, current_cost: float) -> Tuple[bool, str]:
    ...
    if current_cost >= BUDGET_LIMIT_USD:
        logger.warning(f"Input blocked: budget limit reached (${current_cost:.4f} >= ${BUDGET_LIMIT_USD:.2f}).")
        return False, "Your conversation session has reached its budget limit. Please reset the chat or escalate to HR."
    ...
    message_no_accents = "".join(c for c in message_nfd if unicodedata.category(c) != 'Mn')
    ...
    for kw in INJECTION_KEYWORDS:
        if kw in message_lower:
            ...
            return False, "Invalid query: Direct instruction adjustments are not permitted."
    ...
    for pattern in PII_PATTERNS:
        ...
        if valid_matches:
            ...
            return False, "Message blocked: Do not submit password text or credential numbers."
...
def validate_output(answer: str, source_docs: List[Dict[str, Any]]) -> Tuple[bool, str]:
    ...
    is_idk = "don't know" in answer_clean.lower() or "apologize" in answer_clean.lower() or "escalation ticket" in answer_clean.lower()
    if not source_docs and not is_idk:
        logger.warning("Output warning: LLM answered without grounding context. Appending warning.")
